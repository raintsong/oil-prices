...
# --- Sanitize KV_URL for Upstash Python Client ---
raw_url = os.environ.get("KV_URL", "")
clean_url = raw_url
if "@" in raw_url:
    clean_url = "https://" + raw_url.split("@")[1]

redis = Redis(url=clean_url, token=os.environ.get("KV_REST_API_TOKEN"))
...
def get_price(category):
    ...
    if category in yf_map: return jsonify(fetch_ticker_data(yf_map[category], category))
    
    if category == "heating_oil": return jsonify(fetch_eia_v2("petroleum/pri/wfr", "W_EPD2F_PRS_SMA_DPG", category, "weekly", " (Weekly)"))
    if category == "jetfuel": return jsonify(fetch_eia_v2("petroleum/pri/spt", "EER_EPJK_PF4_RGC_DPG", category, "daily", " (Spot)"))
    
    # Algonquin moved to bottom of logic
    if category == "algonquin": return jsonify(fetch_eia_v2("natural-gas/pri/fut", "NG_PRI_FUT_S1_D", category, "daily", " (Spot)"))
